...
namespace RobotController{
    ...
    void FrankaHuskyWrapper::initialize(){        
        ...
        if (ismobile_) {
            ...
            mobileTask2_->setControlledAxes({0, 1, 5}); // planar x, y, yaw(gamma) task for paper-aligned base motion
    ...
    void FrankaHuskyWrapper::compute(const double& time){
        ...
        if (ctrl_mode_ == 11){ //w //identification motion w/o mobile motion, getcalibration available 
            if (mode_change_){
                ...
                if (ismobile_) { 
                    ...
                    H_mobile_ref_ = robot_->getMobilePosition(data_, 2); //2 means state.q_ at base yaw 
        ...
        // Paper-aligned Stage 1 proxy:
        //   1) primary task: EEF velocity from damping control
        //   2) additional task: mobile-base velocity tracks the planar EEF velocity in the null space
        //---------------------------------------------------------------------------------------------------------//
        if (ctrl_mode_ == 30){ //n //cooperative transport
            if (mode_change_){
                //remove
                tsid_->removeTask("task-mobile");
                tsid_->removeTask("task-mobile2");
                ...
                // Priority 0: EEF primary + torque bounds
                tsid_->addMotionTask(*torqueBoundsTask_, 1.0, 0);
                tsid_->addMotionTask(*eeTask_, 1.0, 0);

                // Priority 1: mobile-base additional task + weak posture regularization
                // NOTE: mobileTask_(orientation=true) + eeTask_ together causes divergence in this stack.
                tsid_->addMotionTask(*postureTask_, 1e-6, 1);
                if (ismobile_) {
                    tsid_->addMotionTask(*mobileTask2_, 1.0, 1);
                    mobileTask2_->setDesiredinertia(MatrixXd::Identity(6,6));
                    sampleMobile_.vel.setZero();
                    sampleMobile_.acc.setZero();
                }
                ...
                if (ismobile_) {
                    H_mobile_ref_ = robot_->getMobilePosition(data_, 2);
                    SE3ToVector(H_mobile_ref_, sampleMobile_.pos);
                    sampleMobile_.vel.setZero();
                    sampleMobile_.acc.setZero();
                }
                ...
                // Translational admittance gain in [m/s / N].
                // 5 N -> 0.01 m/s gives a visible but controlled response in simulation.
                Dinv_gain_.setZero();
                Dinv_gain_.head(3).setConstant(0.002);
            }

            // posture task (background, low weight)
            trajPosture_Cubic_->setCurrentTime(time_);
            samplePosture_ = trajPosture_Cubic_->computeNext();
            postureTask_->setReference(samplePosture_);

            // Keep references stationary until the external-force offset is calibrated.
            sampleEE_.vel.setZero();
            sampleEE_.acc.setZero();
            if (ismobile_) {
                sampleMobile_.vel.setZero();
            ...
            if ((!initial_calibration_update_) && (time_ - est_time_ > 0.5)) {
                Fext_calibration_ = Fext_;
                initial_calibration_update_ = true;
                cout << "[Mode 30] Fext calibrated: " << Fext_calibration_.transpose() << endl;
            }

            if (initial_calibration_update_) {
                // EEF damping control in world frame: v_e,d = D^{-1} * (F_h - F_calib).
                Force Fext_global;
                Fext_global = Fext_ - Fext_calibration_;

                Vector3d ee_vel_cmd_world;
                ee_vel_cmd_world(0) = Dinv_gain_(0) * noise_elimination(Fext_global.linear()(0), 1.0);
                ee_vel_cmd_world(1) = Dinv_gain_(1) * noise_elimination(Fext_global.linear()(1), 1.0);
                ee_vel_cmd_world(2) = Dinv_gain_(2) * noise_elimination(Fext_global.linear()(2), 1.0);

                for (int i=0; i<3; i++) {
                    if (ee_vel_cmd_world(i) > 0.05) ee_vel_cmd_world(i) = 0.05;
                    else if (ee_vel_cmd_world(i) < -0.05) ee_vel_cmd_world(i) = -0.05;
                }

                H_ee_ref_.translation() += ee_vel_cmd_world * dt_;
                sampleEE_.vel.head(3) = ee_vel_cmd_world;

                // Additional mobile-base task in the null space:
                // track the planar EEF velocity as much as possible, which is the B(q) intuition when no obstacle
                // constraint is active.
                if (ismobile_) {
                    Vector3d base_vel_cmd_world;
                    base_vel_cmd_world.setZero();
                    base_vel_cmd_world.head(2) = ee_vel_cmd_world.head(2);

                    H_mobile_ref_.translation()(0) += base_vel_cmd_world(0) * dt_;
                    H_mobile_ref_.translation()(1) += base_vel_cmd_world(1) * dt_;
                    sampleMobile_.vel(0) = base_vel_cmd_world(0);
                    sampleMobile_.vel(1) = base_vel_cmd_world(1);
                    sampleMobile_.vel(5) = 0.0;
                }
            }
        ...
        if (ctrl_mode_ == 43){ //d //rotate mobile base CW 90deg (feedback yaw follower)
            if (mode_change_){
                // remove
                tsid_->removeTask("task-mobile");
                tsid_->removeTask("task-mobile2");
                ...
                if (ismobile_) {
                    // V-like fixed-heading follower:
                    // keep translation fixed and close yaw error by feedback.
                    tsid_->addMotionTask(*mobileTask2_, 1.0, 1);
                    mobileTask2_->setDesiredinertia(MatrixXd::Identity(6,6));

                    H_mobile_ref_ = robot_->getMobilePosition(data_, 2);
                    mobile_yaw_init_ = state_.q_(2);
                    mobile_yaw_goal_ = mobile_yaw_init_ - M_PI / 2.0;
                    SE3ToVector(H_mobile_ref_, sampleMobile_.pos);
                    sampleMobile_.vel.setZero();
                    sampleMobile_.acc.setZero();
                }
            ...
            if (ismobile_) {
                const double heading_gain = 1.2;
                const double heading_rate_limit = 0.6;
                const double heading_tolerance = 2.0 * M_PI / 180.0;
                const double psi = state_.q_(2);
                const double heading_error = atan2(sin(mobile_yaw_goal_ - psi), cos(mobile_yaw_goal_ - psi));

                double omega_cmd = heading_gain * heading_error;
                if (omega_cmd > heading_rate_limit) omega_cmd = heading_rate_limit;
                else if (omega_cmd < -heading_rate_limit) omega_cmd = -heading_rate_limit;
                if (fabs(heading_error) < heading_tolerance) omega_cmd = 0.0;

                Matrix3d Rz_step;
                double yaw_step = omega_cmd * dt_;
                rotz(yaw_step, Rz_step);
                H_mobile_ref_.rotation() = H_mobile_ref_.rotation() * Rz_step;

                sampleMobile_.vel.setZero();
                sampleMobile_.acc.setZero();
                sampleMobile_.vel(5) = omega_cmd;
                SE3ToVector(H_mobile_ref_, sampleMobile_.pos);
                mobileTask2_->setReference(sampleMobile_);
            }
        ...
        if (ctrl_mode_ == 44){ //r //rotate mobile base CCW 90deg (feedback yaw follower)
            if (mode_change_){
                // remove
                tsid_->removeTask("task-mobile");
                tsid_->removeTask("task-mobile2");
        ...
        if (ctrl_mode_ == 71){ //y //legacy d-mode: yaw-only goal with fixed translation
            if (mode_change_){
                // remove
                tsid_->removeTask("task-mobile");
                tsid_->removeTask("task-mobile2");
                ...
                if (ismobile_) {
                    // legacy stack: use both mobile orientation task and planar mobile task
                    tsid_->addMotionTask(*mobileTask_, 1.0, 1);
                    mobileTask_->setDesiredinertia(MatrixXd::Identity(6,6));
                    ...
                    SE3 H_init = robot_->getMobilePosition(data_, 2);
                    SE3 H_goal = H_init;

                    // legacy behavior: keep translation fixed, rotate only in yaw
                    H_goal.translation() = H_init.translation();
                    Matrix3d Rz = Eigen::AngleAxisd(-M_PI/2.0, Eigen::Vector3d::UnitZ()).toRotationMatrix();
                    H_goal.rotation() = H_init.rotation() * Rz;

                    trajMobile_Cubic_->setStartTime(time_);
                    trajMobile_Cubic_->setDuration(5.0);
                    trajMobile_Cubic_->setInitSample(H_init);
        ...
        if (ctrl_mode_ == 72){ //u //backup current d-mode: arc-based cubic CW
            if (mode_change_){
                tsid_->removeTask("task-mobile");
                tsid_->removeTask("task-mobile2");
                tsid_->removeTask("task-se3");
                ...
                if (ismobile_) {
                    ...
                    SE3 H_j1_init = robot_->getMobilePosition(data_, 2);
                    double theta = state_.q_(2);

                    Matrix3d R_base = AngleAxisd(theta, Vector3d::UnitZ()).toRotationMatrix();
                    Matrix3d Rz = AngleAxisd(-M_PI/2.0, Vector3d::UnitZ()).toRotationMatrix();

                    Vector3d r_body(0.35, 0.0, 0.0);
                    Vector3d p_base = H_j1_init.translation() - R_base * r_body;
                    Vector3d p_j1_goal = p_base + (R_base * Rz) * r_body;

                    H_mobile_ref_ = H_j1_init;
                    H_mobile_ref_.translation() = p_j1_goal;
                    H_mobile_ref_.rotation() = H_j1_init.rotation() * Rz;

                    trajMobile_Cubic_->setStartTime(time_);
                    trajMobile_Cubic_->setDuration(5.0);
                    trajMobile_Cubic_->setInitSample(H_j1_init);
                    trajMobile_Cubic_->setGoalSample(H_mobile_ref_);
                }
        ...
        if (ctrl_mode_ == 73){ //i //backup current r-mode: arc-based cubic CCW
            if (mode_change_){
                tsid_->removeTask("task-mobile");
                tsid_->removeTask("task-mobile2");
                tsid_->removeTask("task-se3");
        ...
        //---------------------------------------------------------------------------------------------------------//
        //----------- wholebody comparison modes (61: mobile free, 62: mobile hold) ----------------------------//
        //---------------------------------------------------------------------------------------------------------//
        if (ctrl_mode_ == 61){ //1 //wholebody: EE primary + mobile forward + posture low
            if (mode_change_){
                //remove
                tsid_->removeTask("task-mobile");
                tsid_->removeTask("task-mobile2");
                ...
                //mobility - active forward motion at priority 0 (mobile contributes to wholebody)
                if (ismobile_) {
                    tsid_->addMotionTask(*mobileTask2_, 1.0, 0);
                    mobileTask2_->setDesiredinertia(MatrixXd::Identity(6,6));
                    ...
                    trajMobile_Cubic_->setInitSample(H_mobile_ref_);
                    H_mobile_ref_.translation()(0) += 0.05;
                    trajMobile_Cubic_->setGoalSample(H_mobile_ref_);
                }
        ...
        if (ctrl_mode_ == 62){ //2 //arm-only: EE primary + mobile hold + posture low
            if (mode_change_){
                //remove
                tsid_->removeTask("task-mobile");
                tsid_->removeTask("task-mobile2");
                ...
                //mobility - hard hold at priority 0 (mobile locked in place)
                if (ismobile_) {
                    tsid_->addMotionTask(*mobileTask2_, 1.0, 0);
                    mobileTask2_->setDesiredinertia(MatrixXd::Identity(6,6));
        ...
        if (ctrl_mode_ == 63){ //3 //wholebody: EE -0.1x (backward) + mobile backward + posture low
            if (mode_change_){
                tsid_->removeTask("task-mobile");
                tsid_->removeTask("task-mobile2");
                tsid_->removeTask("task-se3");
        ...
        if (ctrl_mode_ == 64){ //4 //arm-only: EE -0.1x (backward) + mobile hold + posture low
            if (mode_change_){
                tsid_->removeTask("task-mobile");
                tsid_->removeTask("task-mobile2");
                tsid_->removeTask("task-se3");
        ...
        if (ctrl_mode_ == 65){ //5 //wholebody: EE diagonal +0.07x+0.07y + mobile forward + posture low
            if (mode_change_){
                tsid_->removeTask("task-mobile");
                tsid_->removeTask("task-mobile2");
                tsid_->removeTask("task-se3");
                ...
                H_ee_ref_ = robot_->position(data_, robot_->model().getJointId("panda_joint7")) * T_offset_;
                trajEE_Cubic_->setInitSample(H_ee_ref_);
                H_ee_ref_.translation()(0) += 0.07;
                H_ee_ref_.translation()(1) += 0.07;
                trajEE_Cubic_->setGoalSample(H_ee_ref_);
                ...
                if (ismobile_) {
                    ...
                    trajMobile_Cubic_->setDuration(2.0);
                    trajMobile_Cubic_->setInitSample(H_mobile_ref_);
                    H_mobile_ref_.translation()(0) += 0.035;
                    trajMobile_Cubic_->setGoalSample(H_mobile_ref_);
                }
                reset_control_ = false;
                mode_change_ = false;
        ...
        if (ctrl_mode_ == 66){ //6 //arm-only: EE diagonal +0.07x+0.07y + mobile hold + posture low
            if (mode_change_){
                tsid_->removeTask("task-mobile");
                tsid_->removeTask("task-mobile2");
                tsid_->removeTask("task-se3");
        ...
        if (ctrl_mode_ == 67){ //7 //wholebody: EE diagonal +0.07x-0.07y + mobile forward + posture low
            if (mode_change_){
                tsid_->removeTask("task-mobile");
                tsid_->removeTask("task-mobile2");
                tsid_->removeTask("task-se3");
        ...
        if (ctrl_mode_ == 68){ //8 //arm-only: EE diagonal +0.07x-0.07y + mobile hold + posture low
            if (mode_change_){
                tsid_->removeTask("task-mobile");
                tsid_->removeTask("task-mobile2");
                tsid_->removeTask("task-se3");
        ...
        if (ctrl_mode_ == 69){ //v //wholebody compliant follow with mobile assist
            if (mode_change_){
                tsid_->removeTask("task-mobile");
                tsid_->removeTask("task-mobile2");
                tsid_->removeTask("task-se3");
                ...
                // Priority 0: EE + mobile hold + torque bounds
                // mobile at P0 forces solver to use ARM for EE tracking
                tsid_->addMotionTask(*torqueBoundsTask_, 1.0, 0);
                tsid_->addMotionTask(*eeTask_, 1.0, 0);
                if (ismobile_) {
                    // Mobile is now an assist task: arm stays primary, base follows.
                    tsid_->addMotionTask(*mobileTask2_, 0.5, 1);
                    mobileTask2_->setDesiredinertia(MatrixXd::Identity(6,6));
                }

                // Priority 1: posture h-bias (increased weight)
                tsid_->addMotionTask(*postureTask_, 0.01, 1);

                // posture trajectory toward h
                if (ismobile_) trajPosture_Cubic_->setInitSample(state_.q_.tail(na_-2));
                else           trajPosture_Cubic_->setInitSample(state_.q_.tail(na_));
                trajPosture_Cubic_->setDuration(2.0);
                trajPosture_Cubic_->setStartTime(time_);
                ...
                eeTask_->setDesiredinertia(MatrixXd::Identity(6,6));
                H_ee_ref_ = robot_->position(data_, robot_->model().getJointId("panda_joint7")) * T_offset_;
                H_ee_init_ = H_ee_ref_; // store initial EE for displacement calculation
                SE3ToVector(H_ee_ref_, sampleEE_.pos);
                sampleEE_.vel.setZero();
                sampleEE_.acc.setZero();
                ...
                mode_change_ = false;
                est_time_ = time_;
                // Zero current bias at mode entry so pressing `v` alone does not
                // create motion from residual measured wrench. External override
                // can still be applied afterwards as a step input.
                Fext_calibration_ = Fext_;
                initial_calibration_update_ = true;

                Dinv_gain_.setZero();
                // EE compliance gain:
                // - increase: arm follows pulling force faster
                // - decrease: arm remains stiffer / slower
                Dinv_gain_.head(3).setConstant(0.01);
            }
            ...
            // A) EE admittance: force -> EE velocity
            Vector3d ee_vel_cmd_world = Vector3d::Zero();
            Force Fext_global;
            Fext_global = Fext_ - Fext_calibration_;
            if (initial_calibration_update_) {
                // Force deadband:
                // - increase: ignore weaker pulling and disturbances
                // - decrease: react to lighter pulling
                ee_vel_cmd_world(0) = Dinv_gain_(0) * noise_elimination(Fext_global.linear()(0), 1.0);
                ee_vel_cmd_world(1) = Dinv_gain_(1) * noise_elimination(Fext_global.linear()(1), 1.0);
                ee_vel_cmd_world(2) = Dinv_gain_(2) * noise_elimination(Fext_global.linear()(2), 1.0);
                ...
                for (int i=0; i<3; i++) {
                    // EE velocity clamp:
                    // - increase: arm follows force more aggressively
                    // - decrease: smoother and safer but slower
                    if (ee_vel_cmd_world(i) > 0.10) ee_vel_cmd_world(i) = 0.10;
                    else if (ee_vel_cmd_world(i) < -0.10) ee_vel_cmd_world(i) = -0.10;
                }
            }
            ...
            // B+C) Mobile follower:
            // - heading is aligned with pulling-force direction
            // - forward motion uses the pulling-force component along current heading
            // - mobile assistance grows as the arm deviates from h
            if (ismobile_) {
                // Heading gain:
                // - increase: faster yaw alignment toward force direction
                // - decrease: slower, smoother turning
                const double heading_gain = 1.0;
                // Heading rate limit:
                // - increase: more visible and quicker yaw motion
                // - decrease: safer but slower base turning
                const double heading_rate_limit = 0.5;
                // Force threshold:
                // - increase: mobile ignores weaker pulling
                // - decrease: mobile reacts to lighter pulling
                const double force_threshold = 2.0;
                // Heading gate:
                // - increase: forward starts earlier even with heading mismatch
                // - decrease: stricter yaw-first behavior
                const double heading_gate_rad = 20.0 * M_PI / 180.0;
                // Base forward gain:
                // - increase: stronger transport-like following
                // - decrease: weaker base translation
                const double forward_gain_base = 0.01;
                // Posture-coupled assist:
                // - increase: mobile helps more aggressively as arm leaves h
                // - decrease: arm carries more before base intervenes
                const double posture_gain_mobile = 0.05;
                // Forward speed limit:
                // - increase: faster base translation
                // - decrease: slower and safer transport motion
                const double forward_speed_limit = 0.25;

                const double psi = state_.q_(2);
                const Vector2d f_xy(Fext_global.linear()(0), Fext_global.linear()(1));
                const double f_norm = f_xy.norm();
                ...
                if (f_norm > force_threshold) {
                    const double psi_des = atan2(f_xy(1), f_xy(0));
                    heading_error = atan2(sin(psi_des - psi), cos(psi_des - psi));

                    omega_cmd = heading_gain * heading_error;
                    if (omega_cmd > heading_rate_limit) omega_cmd = heading_rate_limit;
                    else if (omega_cmd < -heading_rate_limit) omega_cmd = -heading_rate_limit;

                    const Vector2d heading_vec(cos(psi), sin(psi));
                    const double f_parallel = heading_vec.dot(f_xy);
                    const double posture_dev = (state_.q_.tail(na_-2) - q_ref_).norm();
                    const double forward_gain_eff = forward_gain_base + posture_gain_mobile * posture_dev;

                    if (fabs(heading_error) <= heading_gate_rad) {
                        forward_cmd = forward_gain_eff * std::max(0.0, f_parallel);
                        if (forward_cmd > forward_speed_limit) forward_cmd = forward_speed_limit;
                        else if (forward_cmd < 0.0) forward_cmd = 0.0;
                    }
                }

                // convert to world frame
                const Matrix2d R_world_from_base =
                    (Matrix2d() << cos(psi), -sin(psi), sin(psi), cos(psi)).finished();
                const Vector2d forward_world = R_world_from_base * Vector2d(forward_cmd, 0.0);

                H_mobile_ref_.translation()(0) += forward_world(0) * dt_;
                H_mobile_ref_.translation()(1) += forward_world(1) * dt_;
                Matrix3d Rz_step;
                double yaw_step = omega_cmd * dt_;
                rotz(yaw_step, Rz_step);
                H_mobile_ref_.rotation() = H_mobile_ref_.rotation() * Rz_step;
                ...
                sampleMobile_.vel.setZero();
                sampleMobile_.acc.setZero();
                sampleMobile_.vel(0) = forward_world(0);
                sampleMobile_.vel(1) = forward_world(1);
                sampleMobile_.vel(5) = omega_cmd;
                SE3ToVector(H_mobile_ref_, sampleMobile_.pos);
                mobileTask2_->setReference(sampleMobile_);

                // Keep runtime quiet by default; re-enable throttled logging only
                // when actively debugging follower behavior.
            }
        ...
        if (ctrl_mode_ == 70){ //c //arm-only compliant compare
            if (mode_change_){
                tsid_->removeTask("task-mobile");
                tsid_->removeTask("task-mobile2");
                tsid_->removeTask("task-se3");
            ...
            if ((!initial_calibration_update_) && (time_ - est_time_ > 0.5)) {
                Fext_calibration_ = Fext_;
                initial_calibration_update_ = true;
                cout << "[Mode 70] Fext calibrated: " << Fext_calibration_.transpose() << endl;
            }
    ...
    void FrankaHuskyWrapper::base_state(Vector3d & base){
        base(0) = robot_->getMobilePosition(data_, 2).translation()(0); //2 means state.q_ at base yaw 
        base(1) = robot_->getMobilePosition(data_, 2).translation()(1); //2 means state.q_ at base yaw 
        base(2) = atan2(-robot_->getMobilePosition(data_, 2).rotation()(0, 1),robot_->getMobilePosition(data_, 2).rotation()(0,0)); //2 means state.q_ at base yaw 
    }
