...
{   
    ...
    ros::Subscriber ext_wrench_sub = n_node.subscribe("ext_wrench_cmd", 1, &extWrenchCallback, ros::TransportHints().tcpNoDelay(true));
    ...
    if (ismobile_) {
        ...
        base_ref_state_pub_ = n_node.advertise<geometry_msgs::Vector3>("mujoco_ros/mujoco_ros_interface/base_ref_state", 5);
        mobile_tracking_error_pub_ = n_node.advertise<geometry_msgs::Vector3>("mujoco_ros/mujoco_ros_interface/mobile_tracking_error", 5);
        mobile_velocity_error_pub_ = n_node.advertise<geometry_msgs::Vector3>("mujoco_ros/mujoco_ros_interface/mobile_velocity_error", 5);
        mobile_desired_acc_pub_ = n_node.advertise<geometry_msgs::Vector3>("mujoco_ros/mujoco_ros_interface/mobile_desired_acc", 5);
        mobile_command_pub_ = n_node.advertise<geometry_msgs::Vector3>("mujoco_ros/mujoco_ros_interface/mobile_command", 5);
    }
    ...
    while (ros::ok()){        
        ...
        if (ismobile_) {
            ...
            getMobileDebugState();             //reference / error / command traces for plotting
        }
...
void FT_measured_pub() {
    ...
    if (use_Fext_cmd_) {
        Fext_global_ = Fext_cmd_; // terminal override
    } else if (ctrl_->ctrltype() == 69 || ctrl_->ctrltype() == 70) {
        // For v/c follower tests, ignore noisy measured wrench unless an
        // explicit override command is provided. This keeps `v` or `c` from
        // drifting immediately on mode entry.
...
void extWrenchCallback(const geometry_msgs::Wrench::ConstPtr& msg){
    // Override Fext_global_ with terminal-injected wrench (global frame)
    Fext_cmd_(0) = msg->force.x;
    Fext_cmd_(1) = msg->force.y;
    Fext_cmd_(2) = msg->force.z;
    Fext_cmd_(3) = msg->torque.x;
    Fext_cmd_(4) = msg->torque.y;
    Fext_cmd_(5) = msg->torque.z;
    use_Fext_cmd_ = true;
}
...
void getMobileDebugState(){
    Vector3d base_ref, tracking_error, velocity_error, desired_acc, mobile_cmd;
    geometry_msgs::Vector3 base_ref_msg, tracking_error_msg, velocity_error_msg, desired_acc_msg, mobile_cmd_msg;

    ctrl_->base_ref_state(base_ref);
    ctrl_->mobile_tracking_error(tracking_error);
    ctrl_->mobile_velocity_error(velocity_error);
    ctrl_->mobile_desired_acc(desired_acc);
    ctrl_->mobile_command(mobile_cmd);

    base_ref_msg.x = base_ref(0);
    base_ref_msg.y = base_ref(1);
    base_ref_msg.z = base_ref(2);

    tracking_error_msg.x = tracking_error(0);
    tracking_error_msg.y = tracking_error(1);
    tracking_error_msg.z = tracking_error(2);

    velocity_error_msg.x = velocity_error(0);
    velocity_error_msg.y = velocity_error(1);
    velocity_error_msg.z = velocity_error(2);

    desired_acc_msg.x = desired_acc(0);
    desired_acc_msg.y = desired_acc(1);
    desired_acc_msg.z = desired_acc(2);

    mobile_cmd_msg.x = mobile_cmd(0);
    mobile_cmd_msg.y = mobile_cmd(1);
    mobile_cmd_msg.z = mobile_cmd(2);

    base_ref_state_pub_.publish(base_ref_msg);
    mobile_tracking_error_pub_.publish(tracking_error_msg);
    mobile_velocity_error_pub_.publish(velocity_error_msg);
    mobile_desired_acc_pub_.publish(desired_acc_msg);
    mobile_command_pub_.publish(mobile_cmd_msg);
}
...
void keyboard_event(){
    if (_kbhit()){
        ...
        switch (key){
            ...
            case 'n': //cooperative transport: EEF damping + base null-space following (Stage 1)
                msg = 30;
                ctrl_->ctrl_update(msg);
                use_Fext_cmd_ = false; // reset override on mode change
                cout << " " << endl;
                cout << "cooperative transport mode (Stage 1)" << endl;
                cout << " " << endl;
                break;

            //------------- mobile rotation ------------------------------------------------//
            case 'd': //rotate mobile base CW 90deg
                msg = 43;
                ctrl_->ctrl_update(msg);
                cout << " " << endl;
                cout << "rotate mobile base CW 90deg (feedback yaw follower)" << endl;
                cout << " " << endl;
                break;
            case 'y': //legacy rotate mobile base CW 90deg
                msg = 71;
                ctrl_->ctrl_update(msg);
                cout << " " << endl;
                cout << "legacy rotate mobile base CW 90deg (old d-mode logic)" << endl;
                cout << " " << endl;
                break;
            case 'r': //rotate mobile base CCW 90deg
                msg = 44;
                ctrl_->ctrl_update(msg);
                cout << " " << endl;
                cout << "rotate mobile base CCW 90deg (feedback yaw follower)" << endl;
                cout << " " << endl;
                break;
            case 'u': //backup current d-mode logic CW
                msg = 72;
                ctrl_->ctrl_update(msg);
                cout << " " << endl;
                cout << "backup arc-based rotate mobile base CW 90deg" << endl;
                cout << " " << endl;
                break;
            case 'i': //backup current r-mode logic CCW
                msg = 73;
                ctrl_->ctrl_update(msg);
                cout << " " << endl;
                cout << "backup arc-based rotate mobile base CCW 90deg" << endl;
                cout << " " << endl;
                break;

            //------------- wholebody comparison modes ---------------------------------//
            case '1': //wholebody: EE +0.1x with mobile free to assist
                msg = 61;
                ctrl_->ctrl_update(msg);
                cout << " " << endl;
                cout << "wholebody: EE +0.1x primary + mobile additional + posture low" << endl;
                cout << " " << endl;
                break;
            case '2': //arm-only: EE +0.1x with mobile hold
                msg = 62;
                ctrl_->ctrl_update(msg);
                cout << " " << endl;
                cout << "arm-only: EE +0.1x primary + mobile hold + posture low" << endl;
                cout << " " << endl;
                break;
            case '3': //wholebody: EE -0.1x (backward) with mobile free
                msg = 63;
                ctrl_->ctrl_update(msg);
                cout << " " << endl;
                cout << "wholebody: EE -0.1x (backward) + mobile backward + posture low" << endl;
                cout << " " << endl;
                break;
            case '4': //arm-only: EE -0.1x (backward) with mobile hold
                msg = 64;
                ctrl_->ctrl_update(msg);
                cout << " " << endl;
                cout << "arm-only: EE -0.1x (backward) + mobile hold + posture low" << endl;
                cout << " " << endl;
                break;
            case '5': //wholebody: EE diagonal +x+y with mobile free
                msg = 65;
                ctrl_->ctrl_update(msg);
                cout << " " << endl;
                cout << "wholebody: EE diagonal +0.07x+0.07y + mobile forward + posture low" << endl;
                cout << " " << endl;
                break;
            case '6': //arm-only: EE diagonal +x+y with mobile hold
                msg = 66;
                ctrl_->ctrl_update(msg);
                cout << " " << endl;
                cout << "arm-only: EE diagonal +0.07x+0.07y + mobile hold + posture low" << endl;
                cout << " " << endl;
                break;
            case '7': //wholebody: EE diagonal +x-y with mobile free
                msg = 67;
                ctrl_->ctrl_update(msg);
                cout << " " << endl;
                cout << "wholebody: EE diagonal +0.07x-0.07y + mobile forward + posture low" << endl;
                cout << " " << endl;
                break;
            case '8': //arm-only: EE diagonal +x-y with mobile hold
                msg = 68;
                ctrl_->ctrl_update(msg);
                cout << " " << endl;
                cout << "arm-only: EE diagonal +0.07x-0.07y + mobile hold + posture low" << endl;
                cout << " " << endl;
                break;
            case 'v': //wholebody: h-biased compliant follow with mobile assist
                msg = 69;
                ctrl_->ctrl_update(msg);
                use_Fext_cmd_ = false; // reset override on mode change
                cout << " " << endl;
                cout << "wholebody compliant follow: h-posture bias + EE admittance + mobile heading/forward assist" << endl;
                cout << " " << endl;
                break;
            case 'c': //arm-only: h-biased compliant compare
                msg = 70;
                ctrl_->ctrl_update(msg);
                use_Fext_cmd_ = false; // reset override on mode change
                cout << " " << endl;
                cout << "arm-only compliant compare: h-posture bias + EE admittance + mobile hold" << endl;
                cout << " " << endl;
                break;
